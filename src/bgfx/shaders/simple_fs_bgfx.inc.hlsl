...
uniform float4 u_alphaRef;
static float4 u_fogStart;
static float4 u_fogEnd;
static float4 u_fogRange;
static float4 u_fogDisable;
...
void main( float4 gl_FragCoord : SV_POSITION , float4 v_color0 : COLOR , float3 v_texcoord0 : TEXCOORD0 , out float4 bgfx_FragData0 : SV_TARGET0 )
{
float4 bgfx_VoidFrag = vec4_splat(0.0);
float4 col = v_color0*bgfxTexture2D(tex0, float2(v_texcoord0.x, 1.0-v_texcoord0.y));
bgfx_FragData0 = float4(mix(u_fogColor.rgb, col.rgb, v_texcoord0.z), col.a);
DoAlphaTest(col.a);
}
